  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#F5F0E8', opacity: phase === 'fading' ? 0 : 1, transition: phase === 'fading' ? 'opacity 0.6s ease-out' : 'none', gap: '2.5rem' }}>
      ...
      <div style={{ position: 'relative', width: `${SIZE}px`, height: `${SIZE}px` }}>
        <svg viewBox={`0 0 ${SIZE} ${SIZE}`} xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
          <defs>
          ...
          <circle cx={CX} cy={CY} r={R_OUTER} fill="none" stroke="#C9A84C" strokeWidth="1" opacity="0.12" strokeDasharray="3 10" />

          {/* ── Progress arc (clockwise fill) ── */}
          <circle cx={CX} cy={CY} r={R_OUTER} fill="none" stroke="url(#arcGrad)" strokeWidth="1.5" strokeLinecap="round" strokeDasharray={CIRC_OUTER} strokeDashoffset={dashOffset} transform={`rotate(-90 ${CX} ${CY})`} opacity="0.85" />

          {/* ── Leading dot on arc ── */}
          {progress > 0.02 && <circle cx={CX + R_OUTER * Math.cos(progress * 2 * Math.PI - Math.PI / 2)} cy={CY + R_OUTER * Math.sin(progress * 2 * Math.PI - Math.PI / 2)} r="3" fill="#C9A84C" opacity="0.9" />}
          ...
          {Array.from({ length: 36 }).map((_, i) => {
            ...
            return <line key={i} x1={CX + r1 * Math.cos(angle)} y1={CY + r1 * Math.sin(angle)} x2={CX + r2 * Math.cos(angle)} y2={CY + r2 * Math.sin(angle)} stroke="#1A1610" strokeWidth={i % 9 === 0 ? '1' : '0.5'} opacity={i % 9 === 0 ? '0.2' : '0.1'} />;
          })}

          {/* ── Middle rotating ring ── */}
          <g transform={`rotate(${rot3}, ${CX}, ${CY})`}>
            <circle cx={CX} cy={CY} r="56" fill="none" stroke="#C9A84C" strokeWidth="0.5" strokeDasharray="6 20" opacity="0.18" />
          </g>

          {/* ── Inner spinning ring 1 ── */}
          <g transform={`rotate(${rot1}, ${CX}, ${CY})`}>
            <circle cx={CX} cy={CY} r="36" fill="none" stroke="#C9A84C" strokeWidth="0.5" strokeDasharray="3 9" opacity="0.22" />
            {/* 4 dots at cardinal points */}
            {[0, 90, 180, 270].map((deg) => {
              const rad = (deg * Math.PI) / 180;
              return <circle key={deg} cx={CX + 36 * Math.cos(rad)} cy={CY + 36 * Math.sin(rad)} r="1.5" fill="#C9A84C" opacity="0.4" />;
            })}
          </g>

          {/* ── Inner spinning ring 2 (counter) ── */}
          <g transform={`rotate(${rot2}, ${CX}, ${CY})`}>
            <circle cx={CX} cy={CY} r="22" fill="none" stroke="#C9A84C" strokeWidth="0.6" strokeDasharray="2 7" opacity="0.28" />
          </g>
          ...
          {/* ── Percentage counter ── */}
          <text x={CX} y={CY - 10} textAnchor="middle" dominantBaseline="middle" fill="#1A1610" fontSize="11" fontFamily="'Playfair Display', Georgia, serif" opacity="0.45" letterSpacing="1">
            {pct}
          </text>
          <text x={CX} y={CY + 8} textAnchor="middle" dominantBaseline="middle" fill="#1A1610" fontSize="6" fontFamily="'Inter', sans-serif" letterSpacing="3" opacity="0.3">
            LOADING
          </text>
        </svg>
      </div>
