...
const process = [['Define','課題の定義','ユーザー観察と定量データから「本当に解くべき問題」を絞り込む。'],['Design','設計と仮説','アーキテクチャを図示し、技術トレードオフを言語化して意思決定。'],['Build','実装と検証','MVP→計測→改善のサイクルを高速で回す。テストとCI/CDで品質担保。'],['Reflect','振り返りと改善','リリース後の数値変化を分析し、次のイテレーションにフィードバック。']];
...
function renderContent() {
  ...
  $('#skills-grid').innerHTML = Object.entries(skills).map(([group,items]) => `<div class="skill-group"><h3>${group}</h3><ul>${items.map(item => `<li>${item}</li>`).join('')}</ul></div>`).join('');
}

const stars = [[80,120,1.2],[150,60,.8],[230,180,1.5],[310,90,.9],[420,50,1.1],[550,80,.7],[660,140,1.3],[740,70,1],[820,110,.8],[900,50,1.4],[950,200,.9],[930,350,1.2],[880,480,.7],[940,620,1.1],[900,750,.8],[820,870,1.3],[700,940,.9],[560,960,1.2],[400,920,.7],[260,880,1],[130,800,1.4],[60,680,.8],[40,550,1.1],[70,400,.9],[50,260,1.2],[180,380,.6],[300,280,.9],[450,160,.7],[620,220,1],[770,290,.8],[860,380,.7],[750,460,.5],[650,380,.8],[340,440,.6],[190,500,.7]];
function backgroundSvg() {
  const starMarkup = stars.map(([x,y,r],i) => `<circle cx="${x}" cy="${y}" r="${r}" class="star star-${i}">`).join('');
  const ticks = Array.from({length:24},(_,i) => { const a=i/24*Math.PI*2, r1=378, r2=i%6===0?368:373; return `<line x1="${500+r1*Math.cos(a)}" y1="${500+r1*Math.sin(a)}" x2="${500+r2*Math.cos(a)}" y2="${500+r2*Math.sin(a)}" class="gold-line">`; }).join('');
  const rays = Array.from({length:12},(_,i) => { const a=i/12*Math.PI*2,r1=36,r2=i%3===0?58:46; return `<line x1="${r1*Math.cos(a)}" y1="${r1*Math.sin(a)}" x2="${r2*Math.cos(a)}" y2="${r2*Math.sin(a)}" class="sun-ray">`; }).join('');
...
function loadingSvg(progress) {
  const size=180, center=90, radius=78, circumference=2*Math.PI*radius, offset=circumference*(1-progress), ticks=Array.from({length:36},(_,i)=>{const a=i/36*Math.PI*2-Math.PI/2,r1=83,r2=83+(i%9===0?11:8);return `<line x1="${center+r1*Math.cos(a)}" y1="${center+r1*Math.sin(a)}" x2="${center+r2*Math.cos(a)}" y2="${center+r2*Math.sin(a)}" class="loading-tick"/>`}).join('');
...
function updateTheme() {
  ...
  const lerp=(a,b)=>Math.round(a+(b-a)*t); const bg=`${lerp(245,13)},${lerp(240,12)},${lerp(232,10)}`; const fg=`${lerp(26,237)},${lerp(22,232)},${lerp(16,222)}`;
  document.documentElement.style.setProperty('--bg',bg); document.documentElement.style.setProperty('--fg',fg); document.documentElement.style.setProperty('--t',t);
  $('.scroll-background').style.setProperty('--rotation',`${t*180}deg`); $('.scroll-background').style.setProperty('--middle-rotation',`${t*-108}deg`); $('.scroll-background').style.setProperty('--scale-tilt',`${Math.sin(t*Math.PI)*8}deg`);
  $('.scroll-background .scale').style.transform=`translate(500px,500px) rotate(${Math.sin(t*Math.PI)*8}deg)`;
}
function startLoading() {
  const screen=$('#loading-screen'), ring=$('#loading-ring'), rule=$('#loading-rule'), start=performance.now(), duration=2400;
  function frame(now){const p=Math.min((now-start)/duration,1), eased=p<.5?4*p**3:1-(-2*p+2)**3/2; ring.innerHTML=loadingSvg(eased); rule.style.width=`${eased*120}px`; if(p<1) requestAnimationFrame(frame); else {screen.classList.add('fade');setTimeout(()=>screen.remove(),600)}} requestAnimationFrame(frame);
}
renderContent(); $('#scroll-background').innerHTML=backgroundSvg(); window.addEventListener('scroll',updateTheme,{passive:true}); window.addEventListener('resize',updateTheme); updateTheme(); startLoading();
